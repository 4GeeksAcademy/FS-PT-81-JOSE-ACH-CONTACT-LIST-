const getState = ({ getStore, getActions, setStore }) => {
    return {
        ...
        actions: {
            createUser: async () => {
                try {
                    const response = await fetch("https://playground.4geeks.com/contact/agendas/joseach", {
                        method: "POST"
                    });
                    if (!response.ok) {
            ...
            getInfoContacts: async () => {
                try {
                    const response = await fetch("https://playground.4geeks.com/contact/agendas/joseach/contacts", {
                        method: "GET"
                    });
            ...
            deleteContact: async (id) => {
                try {
                    const response = await fetch(`https://playground.4geeks.com/contact/agendas/joseach/contacts/${id}`, {
                        method: "DELETE"
                    });
                    if (!response.ok) {
